...
if __name__ == '__main__':
    working_path = "./data/it/IBSMesh_400_2_OnGivenPointCloudWeightedSampler_5_500_OnGivenPointCloudWeightedSampler_512/reaching_out_low"
    affordance_name = "reaching_out_low"
    object_name = "human_reaching_out_low"
    tests = DeglomeratorClearance(working_path, affordance_name, object_name)

    tri_mesh_obj = trimesh.load_mesh(tests.object_filename())

    env_filename = os.path.join(tests.working_path, tests.affordance_name + "_" + tests.object_name + "_environment.ply")
    ...
    angle = (2 * math.pi / tests.num_orientations)

    for ori in range(tests.num_orientations):
        idx_from = ori * tests.sample_size
        idx_to = idx_from + tests.sample_size
        pv_points = tests.pv_points[idx_from:idx_to]
        pv_vectors = tests.pv_vectors[idx_from:idx_to]

        idx_from = ori * tests.sample_clearance_size
        idx_to = idx_from + tests.sample_clearance_size
        cv_points = tests.cv_points[idx_from:idx_to]
        cv_vectors = tests.cv_vectors[idx_from:idx_to]
